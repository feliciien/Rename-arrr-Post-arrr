    ...
    def run(self):
        ...
        for index, filename in enumerate(self.files, start=1):
            if not self._is_running:
                break  # Allows for future cancellation feature
            try:
                title, year = extract_title_year(filename)
                metadata = rename_files(self.folder_path, title, year)
                new_filename = f"{metadata['title']} ({metadata['year']}).{filename.split('.')[-1]}"
